...
public class SuzukiKasami {
    ...
    void simulate() {
        while (true) {
            ...
            currentLNs[currentTokenHolder - 1] = currentRNs[currentTokenHolder - 1];
            ...
            if (!tokenQueue.isEmpty()) {
                ...
                System.out.println("Token sent from Site " + currentTokenHolder + " to Site " + nextTokenHolder);
    ...
    void requestCriticalSection(int requestingSiteId) {
        ...
        for (int i = 0; i < numSites; i++) {
            if (i != requestingSiteId - 1) {
                sites[i].RN[requestingSiteId - 1] = Math.max(sites[i].RN[requestingSiteId - 1],
                        sites[requestingSiteId - 1].RN[requestingSiteId - 1]);

                if (currentTokenHolder == i + 1 && sites[i].RN[requestingSiteId - 1] == sites[i].LN[requestingSiteId - 1] + 1) {
                    System.out.println("Token sent from Site " + currentTokenHolder + " to Site " + requestingSiteId);
